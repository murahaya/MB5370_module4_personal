---
title: "MB5370 Module 04. Workshop 1"
author: "Hayato Murai"
...
#nrow is where you specify the number of rows in a facet wrap 
#ncol is where you specify the number of columns in a facet wrap
...
```{r}
# R uses a specified formula to plot the line. Default is Loess and y = x formula
```
...
# The "se = FALSE" hides the shaded confidence interval around the line
...
ggplot(data=mpg, mapping = aes(x = displ, y = hwy, group=class, fill=class))+ 
  #geom_smooth()
  geom_boxplot()
  #geom_histogram()
  #geom_area()
...
# These graphs will look identical and display the same data in the same way.
# The first graphing method is better because it uses fewer characters and 
# lines of code 
```
...
```{r}
# Y axis is specified so the default "count" is not used 
```
...
```{r}
ggplot(data = diamonds) + 
  geom_bar(mapping = aes(x = cut, colour = cut)) #not really visible
ggplot(data = diamonds) + 
...
```{r}
ggplot(data = mpg)+
  geom_point(mapping = aes(x = displ, y = hwy)) #Lots of hidden overlapped data

ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy), position = "jitter") # Jittered data overlaps less
  
```

# Layered Grammar of Graphics

Classic GGplot Template
